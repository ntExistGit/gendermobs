...
public class RenderMoose extends MobRenderer<EntityMoose, ModelMoose> {
    private static final ResourceLocation TEXTURE_ANTLERED = new ResourceLocation(
            "alexsmobs:textures/entity/moose_antlered.png");
    private static final ResourceLocation TEXTURE_SNOWY_ANTLERED = new ResourceLocation(
            "alexsmobs:textures/entity/moose_snowy_antlered.png");
    private static final ResourceLocation TEXTURE_SNOWY = new ResourceLocation(
            "alexsmobs:textures/entity/moose_snowy.png");
    ...
    class LayerSnow extends RenderLayer<EntityMoose, ModelMoose> {
        public LayerSnow(RenderMoose this$0) {
            super(this$0);
        }

        public void render(PoseStack matrixStackIn, MultiBufferSource bufferIn, int packedLightIn,
                EntityMoose entitylivingbaseIn, float limbSwing, float limbSwingAmount, float partialTicks,
                float ageInTicks, float netHeadYaw, float headPitch) {
            if (entitylivingbaseIn.isSnowy()) {
                VertexConsumer ivertexbuilder = bufferIn.getBuffer(
                        RenderType.entityCutoutNoCull(entitylivingbaseIn.isAntlered() && !entitylivingbaseIn.isBaby()
                                ? RenderMoose.TEXTURE_SNOWY_ANTLERED
                                : RenderMoose.TEXTURE_SNOWY));
                ((ModelMoose) this.getParentModel()).renderToBuffer(matrixStackIn, ivertexbuilder, packedLightIn,
